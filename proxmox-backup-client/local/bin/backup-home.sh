...
usage_info() {
  ...
  exit_code="$1"
  echo -e "Usage:\n"
  echo -e "\t${script_name} [OPTIONS] <HOSTNAME>\n"
  echo -e "Options:\n"
...
if [ -e "$PASSFILE" ]; then
  ...
  # user that this script is initiated from.
    ...
    #exit 4
  #fi
  # IMPORTANT(JEFF): The following variables may be overwritten at this time,
  #
  # REPOSITORY
  # PASSPHRASE
  # NAMESPACE
...
if [[ -n "$EXCLUSIONS" ]] && [[ "$EXCLUSIONS" != "" ]]; then
  ...
  echo "INFO: Not using an exclusion list for this backup..."
  echo
fi
...
if [ -z "$NAMESPACE" ]; then # NAMESPACE variable is NOT declared
  ...
  # NAMESPACE variable is declared in configuration
